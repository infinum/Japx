...
public final class JapxEncoder {
    ...
    public let options: JapxCore.Encoder.Options
    
    /// Initializes `self` with underlying `JSONEncoder` instance
    public init(jsonEncoder: JSONEncoder = JSONEncoder(), options: JapxCore.Encoder.Options = .default) {
    ...
    public func encode<T>(_ value: T, additionalParams: Parameters? = nil) throws -> Parameters where T : Encodable {
        ...
        return try JapxCore.Encoder.encode(data: data, additionalParams: additionalParams, options: options)
    }
}
...
public final class JapxDecoder {
    ...
    public init(jsonDecoder: JSONDecoder = JSONDecoder(), options: JapxCore.Decoder.Options = .default) {
    ...
    public func decode<T>(_ type: T.Type, from json: Parameters, includeList: String? = nil) throws -> T where T : Decodable {
        let data = try JapxCore.Decoder.data(withJSONAPIObject: json, includeList: includeList, options: options)
    ...
    public func decode<T>(_ type: T.Type, from data: Data, includeList: String? = nil) throws -> T where T : Decodable {
        let data = try JapxCore.Decoder.data(with: data, includeList: includeList, options: options)
        return try jsonDecoder.decode(type, from: data)
    }
}
