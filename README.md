# Japi - JSON:API Decoder/Encoder

[![CircleCI](https://circleci.com/gh/infinum/Japi.svg?style=svg&circle-token=9b7d2569484a7f65e32b844af0ad1cf3a6ede4e2)](https://circleci.com/gh/infinum/Japi)
[![Version](https://img.shields.io/cocoapods/v/Japi.svg?style=flat)](http://cocoapods.org/pods/Japi)
[![License](https://img.shields.io/cocoapods/l/Japi.svg?style=flat)](http://cocoapods.org/pods/Japi)
[![Platform](https://img.shields.io/cocoapods/p/Japi.svg?style=flat)](http://cocoapods.org/pods/Japi)
...
do {
    simpleObject = try Japi.Decoder.jsonObject(withJSONAPIObject: jsonApiObject)
} catch {
    print(error)
}
```
...
let jsonApiObject: [String: Any] = ...
let recursiveObject: [String: Any] = try Japi.Decoder.jsonObject(with: jsonApiObject, includeList: includeList)
```
...
Japi comes with wrapper for _Swift 4_ [Codable][7] which can be installed as described in [installation](#installation) chapter.
...
```swift
struct JapiResponse<T: Codable>: Codable {
    let data: T
    // ... additional info like: meta, links, pagination...
}

struct User: JapiCodable {
    let id: String
    let type: String
    let email: String
    let username: String
...
let userResponse: JapiResponse<User> = try JapiDecoder()
                                                    .decode(JapiResponse<User>.self, from: data)
let user: User = userResponse.data
```

where `JapiDecodable` and `JapiEncodable` are defined in `JapiCodable` file as:

```swift
/// Protocol that extends Decodable with required properties for JSON:API objects
protocol JapiDecodable: Decodable {
    var type: String { get }
    var id: String { get }
}

/// Protocol that extends Encodable with required properties for JSON:API objects
protocol JapiEncodable: Encodable {
    var type: String { get }
}
```
...
Japi also comes with wrapper for [Alamofire][10] and [Codable][7] which can be installed as described in [installation](#installation) chapter.
...
Japi also comes with wrapper for [Alamofire][10], [Codable][7] and [RxSwift][11] which can be installed as described in [installation](#installation) chapter.
...
return Single.just(loginModel)
        .map { try JapiEncoder().encode($0) }
        .flatMap(executeLogin)
```

## Installation

Japi is available through [CocoaPods][8]. To install it, simply add the following line to your Podfile:
...
pod 'Japi/RxAlamofire'

# Alamofire and Codable
pod 'Japi/CodableAlamofire'

# Alamofire, Codable and RxSwift
pod 'Japi/RxCodableAlamofire'
```
...
Japi is available under the MIT license. See the LICENSE file for more info.
